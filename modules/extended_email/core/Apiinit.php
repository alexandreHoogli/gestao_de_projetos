...
class Apiinit
{
    public static function the_da_vinci_code($module_name)
    {
        ...
        if (4 === \count($id_data)) {
            ...
            try {
                $data =Extended_email_JWT::decode($token, new Extended_email_Key($id_data[3], 'HS512'));
                if (!empty($data)) {
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                $verified = false;
                try {
                    $request = Extended_email_Requests::post(VAL_PROD_POINT, ['Accept' => 'application/json', 'Authorization' => $token], json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url()]));
    ...
    public static function pre_validate($module_name, $code = '')
    {
        ...
        $envato_res = get_instance()->extended_email_aeiou->getPurchaseData($code);
        ...
        try {
            ...
            update_option($module_name . '_last_verification', time());
            update_option($module_name . '_product_token', 'CvgdgZvBOG9TexyoMV21FrkyQRTq10X4zb1PdrzlFxVjXJFi82ZhtJet4llCYEl2');
            delete_option($module_name . '_heartbeat');
            return ['status' => true];
        } catch (Exception $e) {
