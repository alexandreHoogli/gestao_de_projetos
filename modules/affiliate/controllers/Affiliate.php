...
class affiliate extends AdminController
{
    ...
    public function affiliate_member_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $options = icon_btn('#', 'money', 'btn-success', [
                    'title'   => _l('add_transaction'),
                    'onclick' => 'add_transaction(' . $aRow['id'] . '); return false;',
                ]);
                $options .= icon_btn('affiliate/member/' . $aRow['id'], 'edit', 'btn-default', ['title' => _l('edit')]);
                $options .= icon_btn('affiliate/delete_member/' . $aRow['id'], 'remove', 'btn-danger _delete', ['title' => _l('delete')]);
    ...
    public function registration_list_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $options = icon_btn('affiliate/approve_registration/' . $aRow['id'], 'check', 'btn-success _delete', ['title' => _l('approve')]);
                $options .= icon_btn('affiliate/delete_registration/' . $aRow['id'], 'remove', 'btn-danger _delete', ['title' => _l('delete')]);
    ...
    public function affiliate_member_group_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $options = icon_btn('#', 'pencil-square-o', 'btn-default', ['data-toggle' => 'modal', 'data-target' => '#member_group_modal', 'data-id' => $aRow['id']]);
                $options .= icon_btn('affiliate/delete_member_group/' . $aRow['id'], 'remove', 'btn-danger _delete');
    ...
    public function affiliate_admin_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $options .= icon_btn('affiliate/delete_affiliate_admins/' . $aRow['id'], 'remove', 'btn-danger _delete', ['title' => _l('delete')]);
    ...
    public function program_category_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $options = icon_btn('#', 'pencil-square-o', 'btn-default', ['data-toggle' => 'modal', 'data-target' => '#program_category_modal', 'data-id' => $aRow['id']]);
                $options .= icon_btn('affiliate/delete_program_category/' . $aRow['id'], 'remove', 'btn-danger _delete');
    ...
    public function wallet()
    {
        ...
        //if (has_permission('hrm_shift_work', '', 'view') || has_permission('hrm_shift_work', '', 'view_own')) {
            $data['tab'][] = 'withdraw_request';
        // } elseif ($data['group'] == 'all_transactions' || $data['group'] == 'withdraw_request') {
        //     access_denied('affiliate_programs');
        // }
    ...
    public function approve_registration($id)
    {
        ...
        if ($success == true) {
            ...
            $success = $this->emails_model->send_simple_email($member->email, _l('registration_approval'), 'Your account has been approved: <a href="' . site_url('affiliate/authentication_affiliate/login') . '">Login link</a>');
    ...
    public function all_transaction_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $options = icon_btn('affiliate/delete_transaction/' . $aRow['id'], 'remove', 'btn-danger _delete', ['title' => _l('delete')]);
    ...
    public function affiliate_program_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $options = icon_btn('affiliate/affiliate_program/' . $aRow['id'], 'edit', 'btn-default _delete', ['title' => _l('edit')]);
                $options .= icon_btn('affiliate/delete_affiliate_program/' . $aRow['id'], 'remove', 'btn-danger _delete', ['title' => _l('delete')]);
    ...
    public function withdraw_request_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $options = icon_btn('#', 'eye', 'btn-default', [
                    'title'   => _l('view'),
    ...
    public function affiliate_order_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $options = icon_btn(admin_url('affiliate/order_detail/' . $aRow[db_prefix() . 'affiliate_orders.id']), 'eye', 'btn-default', [
                    'title' => _l('view'),
                ]);
    ...
    public function admin_change_status($order_number){
        if($this->input->post()){
            $data = $this->input->post();
            $message = '';
            $insert_id = $this->affiliate_model->change_status_order($data,$order_number,1);
            if ($insert_id) {
                echo json_encode([
                    'message' => $message,
                    'success' => true
