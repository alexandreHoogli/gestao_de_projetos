...
class Affiliate_model extends App_Model
{
    ...
    public function add_affiliate_program($data)
    {
        ...
        if (!isset($data['commission_first_invoices'])) {
            $data['commission_first_invoices'] = 'disable';
        }
    ...
    public function commission_new_registrantion($member_id)
    {
        ...
        if ($affiliate_program) {
            $this->db->insert(db_prefix() . 'affiliate_logs', [
                ...
                'datecreated' => date('Y-m-d H:i:s'),
                'member_id'   => $member->under_affiliate,
            ]);
            $insert_id = $this->db->insert_id();
            if ($insert_id) {
                $affiliate_logs = $this->get_affiliate_logs('', ['type' => 'new_registration', 'member_id' => $member->under_affiliate, 'program_id' => $affiliate_program->id]);
                ...
                if ($commission > 0) {
                    $this->db->where('affiliate_program_id', $affiliate_program->id);
                    $this->db->where('member_id', $member->under_affiliate);
                    $this->db->where('status', 0);
                ...
                if ($affiliate_program->enable_commission == 'enable') {
                    ...
                    if ($affiliate_program->commission_policy_type == '2') {
                        if ($affiliate_program->commission_type == 'percentage') {
                            ...
                            if ($affiliate_program->commission_first_invoices == 'enable') {
     ...
     */
    public function get_first_invoices($member_id, $invoiceid, $max = 0, $affiliate_program, $commission = 0)
    {
     ...
     * @return     string  The item name.
     */
    public function get_item_id_by_program($program_id)
    {
        $items   = [];
        ...
        if ($program->enable_discount == 'enable') {
            ...
            if ($program->discount_products != '') {
                $where .= 'FIND_IN_SET(id, "' . $program->discount_products . '")';
            }
            if ($program->discount_product_groups != '') {
                if ($where == '') {
                    $where .= 'FIND_IN_SET(group_id, "' . $program->discount_product_groups . '")';
                } else {
        ...
        if ($program->enable_commission == 'enable' && $program->commission_affiliate_type != 2) {
            ...
            if ($program->commission_products != '') {
                $where .= 'FIND_IN_SET(id, "' . $program->commission_products . '")';
            }
            if ($program->discount_product_groups != '') {
                if ($where == '') {
                    $where .= 'FIND_IN_SET(group_id, "' . $program->commission_product_groups . '")';
                } else {
     ...
     */
    public function get_product_list($member_id = '')
    {
        if ($member_id == '') {
            $member_id = get_affiliate_user_id();
        }
    ...
    public function get_my_product_select($member_id, $where = [], $select = true)
    {
        ...
        foreach ($my_programs as $k => $val) {
            $product = $this->affiliate_model->get_item_id_by_program($val['id']);
            if (isset($product['discount'])) {
                foreach ($product['discount'] as $value) {
                    if ($where_product == '') {
                        $where_product = $value['id'];
                    } else {
                        $where_product .= ',' . $value['id'];
                    }
                }
            ...
            if (isset($product['commission'])) {
                foreach ($product['commission'] as $value) {
                    if ($where_product == '') {
                        $where_product = $value['id'];
                    } else {
                        $where_product .= ',' . $value['id'];
        ...
        if ($where_product != '') {
            $this->db->where('id in (' . $where_product . ')');
        }
